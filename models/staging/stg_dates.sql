...
create_date_table as (
    select
        ...
        concat(monthname(date_day), ' ', date_part('year', date_day)) as month_and_year,
        ...
        date_trunc('week', date_day) + interval '1 week' - interval '1 day' as week_end_date,
        isodow(date_day) as week_day_number,
        date_diff('week', current_date, date_day) as week_offset,
        if(date_diff('week', current_date, date_day) < 0, 1, 0) as week_completed,
        ...
        /*has 53 weeks*/
        if(week(date_trunc('year', date_day) + interval '1 year' - interval '1 day') = 53, 1, 0) as has_53_iso_weeks
    from date_spine
